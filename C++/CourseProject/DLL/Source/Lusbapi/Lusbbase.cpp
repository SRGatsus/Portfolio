...
BOOL WINAPI TLUSBBASE::GetModuleNameByLength(PCHAR const ModuleName, WORD ModuleNameLength)
{
	...
	// виртуальный слот доступен?
	if(hDevice == INVALID_HANDLE_VALUE) { printf("error 7"); LAST_ERROR_NUMBER(7); return FALSE; }
	// проверим строку
	else if(!ModuleName) { printf("error 8"); LAST_ERROR_NUMBER(8); return FALSE; }
	...
	if(ModuleNameLength)
	{
		...
		if(!LDeviceIoControl(DIOC_SEND_COMMAND, InBuf, sizeof(InBuf), ModuleName, ModuleNameLength, TimeOut)) { printf("error 32qweq3"); LAST_ERROR_NUMBER(9); return FALSE; }
	}
	else
	{
		...
		if(!LDeviceIoControl(DIOC_SEND_COMMAND, InBuf, sizeof(InBuf), LocModuleName, LONG_MODULE_NAME_STRING_LENGTH, TimeOut))
		{
			...
			if (!LDeviceIoControl(DIOC_SEND_COMMAND, InBuf, sizeof(InBuf), LocModuleName, SHORT_MODULE_NAME_STRING_LENGTH, TimeOut)) { printf("error 323"); LAST_ERROR_NUMBER(9); return FALSE; }
