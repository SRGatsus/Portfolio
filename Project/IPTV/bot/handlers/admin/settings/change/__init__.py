...
def setup(dp: Dispatcher):
    ...
    dp.register_message_handler(check_input, content_types=["text", "document"],
                                state=[AdminStates.change_settings,AdminStates.change_settings_check_input])
    dp.register_callback_query_handler(init_input, lambda c: str(c.data).startswith("s_change"),
                                       state=[AdminStates.settings,AdminStates.change_settings])

    dp.register_callback_query_handler(confirmation_input, lambda c: c.data == "yes",
                                       state=AdminStates.change_settings_check_input)
