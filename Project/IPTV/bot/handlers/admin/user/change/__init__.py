...
def setup(dp: Dispatcher):
    ...
    dp.register_message_handler(check_input, content_types=["text", "document"],
                                state=[AdminStates.change_user,AdminStates.change_user_check_input])
    dp.register_callback_query_handler(init_input, lambda c: str(c.data).startswith("u_change"),
                                       state=[AdminStates.view_user,AdminStates.change_user])

    dp.register_callback_query_handler(confirmation_input, lambda c: c.data == "yes",
                                       state=AdminStates.change_user_check_input)
